...
export default function ClassDropdown() {
  ...
  const selectedClassroom = classrooms.find(
    (c) => c.classRoom.id === selectedClassId
  );

  return (
    <Select value={selectedClassId} onValueChange={handleValueChange}>
      <SelectTrigger className="w-full max-w-80 cursor-pointer rounded-xl border border-gray-200 bg-orange-500 px-4 py-6 text-sm shadow-sm transition-all hover:border-black focus:ring-0">
        <SelectValue>
          {selectedClassId === 'all'
            ? 'All Classes'
