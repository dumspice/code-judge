...
Checklist theo **thứ tự nên làm** (base → cấu hình → schema → API/worker/FE). Đánh dấu `[x]` khi hoàn thành. Tham chiếu schema: `apps/core-api/prisma/schema.prisma`, tài liệu: `apps/core-api/prisma/schema.md`.
...
| # | Việc | Ghi chú |
|---|------|---------|
| A.1 | Cấu trúc monorepo `apps/core-api`, `apps/worker`, `apps/web` | npm workspaces |
| A.2 | `package.json` root: script `dev`/`build`/`lint` gọi từng app | |
| A.3 | Docker Compose: Postgres, Redis, (MinIO nếu cần upload) | `docker-compose.yml` |
| A.4 | `.env.example` cho từng app + root | Đã có mẫu; bổ sung khi thêm service |
| A.5 | README hướng dẫn chạy local (DB, migrate, 3 process) | `README.md` |
...
| # | Việc | Ghi chú |
|---|------|---------|
| B.1 | `prisma.config.ts` + `DATABASE_URL` | Prisma 7 |
| B.2 | NestJS `ConfigModule` (`.env`, `.env.local`) | `AppModule` |
| B.3 | ESLint + Prettier cho repo | |
| B.4 | CI: lint + build `core-api`, `worker`, `web` | Task G0-5 trong backlog chi tiết |
| B.5 | Health endpoint (`/health`: DB, Redis) | Task G0-2 |
| B.6 | Swagger/OpenAPI (`/api-docs`) khi không production | Đã có cơ bản |
...
| # | Việc | Ghi chú |
|---|------|---------|
| C.1 | Hoàn thiện `schema.prisma` (Org, User, Class, Problem, Contest, Submission, …) | Theo RFP |
| C.2 | Sinh migration + `migrate deploy` trên DB sạch | Thư mục `prisma/migrations/` |
| C.3 | `prisma generate` trong pipeline / sau pull | |
| C.4 | Seed tối thiểu: admin/instructor/student, org mẫu, problem `PUBLIC` + `TestCase` | `prisma/seed.ts` (nếu chưa có thì thêm) |
| C.5 | Tài liệu DBML / giải thích trường | `prisma/schema.md` |
...
| # | Việc | Ghi chú |
|---|------|---------|
| D.1 | `PrismaModule` + `PrismaService` (adapter pg) | |
| D.2 | Envelope response + exception filter thống nhất | Đã có interceptor/filter |
| D.3 | `ValidationPipe` global (whitelist DTO) | `main.ts` |
| D.4 | CORS, port `PORT` | |
| D.5 | Module realtime (Socket.io) + gateway mẫu submission | `RealtimeModule` |
...
| # | Việc | Ghi chú |
|---|------|---------|
| E.1 | Đăng ký / đăng nhập email + `passwordHash` (bcrypt) | Mở rộng `AuthService` |
| E.2 | JWT access token; `JwtStrategy` + `JwtAuthGuard` global | Đã có khung |
| E.3 | `@Public()` cho route không cần token | |
| E.4 | `@Roles()` + `RolesGuard` (Role: ADMIN, INSTRUCTOR, STUDENT) | |
| E.5 | `GET /auth/me` trả user đầy đủ (kèm org membership nếu cần) | |
| E.6 | `User.isActive` — chặn login khi false | |
| E.7 | `InstructorVerificationStatus` — flow duyệt GV | |
| E.8 | OAuth Google — model `OAuthAccount`, callback | |
| E.9 | Socket: handshake JWT (bỏ query `userId` thô) | Task G0-4 |
...
| # | Việc | Ghi chú |
|---|------|---------|
| F.1 | CRUD `Organization` (ADMIN hoặc ORG_ADMIN) | |
| F.2 | Mời user vào org — `OrganizationMembership` + `OrgRole` | |
| F.3 | API import danh sách email (batch) | |
| F.4 | Policy list `Problem` với `ORG_INTERNAL` + `organizationId` | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| G.1 | CRUD `ClassRoom` (`classCode` unique, `ownerId`, `organizationId?`) | |
| G.2 | `ClassEnrollment`: join bằng `classCode`, trạng thái PENDING/ACTIVE/REMOVED | |
| G.3 | `ClassInvite`: tạo token, hết hạn, đánh dấu `usedAt` | |
| G.4 | GV gỡ học viên khỏi lớp | |
| G.5 | `ClassAssignment`: gắn `problemId` hoặc `contestId` + `dueAt` | |
| G.6 | FE: màn hình lớp (tạo, copy mã, danh sách SV, bài tập) | `apps/web` |
...
| # | Việc | Ghi chú |
|---|------|---------|
| H.1 | CRUD Problem (slug, visibility, org, limits, mode) | |
| H.2 | Gắn `Tag` / `ProblemTag` | |
| H.3 | CRUD `TestCase` (order, hidden, weight); validate ≤ `maxTestCases` | |
| H.4 | API list/filter practice: chỉ `PUBLIC` (+ quyền org cho INTERNAL) | |
| H.5 | Ẩn input/output test `isHidden` với thí sinh | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| I.1 | `BullMqModule`: Redis `maxRetriesPerRequest: null`, `Queue` + `QueueEvents` | |
| I.2 | `POST /submissions`: tạo `Submission` + add job (metadata `submissionId`, priority) | |
| I.3 | Set `context`, `contestId`, `classRoomId`, `judgePriority` đúng nghiệp vụ | |
| I.4 | `BullMqEventsService`: progress / completed / failed → cập nhật realtime | |
| I.5 | Retry/backoff job; dead-letter hoặc requeue | |
| I.6 | Auth: `userId` lấy từ JWT, không tin body | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| J.1 | Worker BullMQ consume queue trùng tên với API | `apps/worker` |
| J.2 | Đọc `Submission` + `TestCase`; chạy sandbox (Docker) | |
| J.3 | Giới hạn CPU/RAM/time theo Problem / ContestProblem override | |
| J.4 | Ghi `Submission.status`, `caseResults` (JSON), `testsPassed`/`testsTotal`, logs | |
| J.5 | Ưu tiên job: đọc `judgePriority` / `context === CONTEST` | |
| J.6 | Tách module judge (runner, compile, compare output) cho dễ test | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| K.1 | CRUD `Contest` (slug, time window, password hash, policy, max submissions) | |
| K.2 | `ContestProblem` (điểm, override time/memory) | |
| K.3 | `ContestParticipant` + vào phòng (verify password) | |
| K.4 | Enforcement: chỉ submit trong [startAt, endAt]; check attempt vs max | |
| K.5 | Trả kết quả theo `ContestTestFeedbackPolicy` (SUMMARY vs VERBOSE) | |
| K.6 | FE: tạo contest, join, countdown, IDE submit | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| L.1 | Lưu `GoldenSolution` (language, code, isPrimary) | |
| L.2 | Upload đề → `AiGenerationJob` + storage file (`inputDocUrl`) | |
| L.3 | Service gọi LLM → parse test đề xuất → lưu `structuredOutput` | |
| L.4 | Pipeline: chạy golden trên input → điền `expectedOutput` vào `TestCase` | |
| L.5 | API duyệt / sửa / xóa test trước khi publish | |
| L.6 | Rate limit / quota AI | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| M.1 | Aggregate pass rate, ranking (theo contest / lớp) từ `Submission` | |
| M.2 | `ReportExport`: job async XLSX/PDF + `fileUrl` | |
| M.3 | `CodeSimilarityFinding`: job so cặp submission (AST/heuristic) | |
| M.4 | `Certificate`: phát hành sau contest/lớp | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| N.1 | Cấu hình `NEXT_PUBLIC_CORE_URL`, client gọi API có cookie/token | |
| N.2 | Trang đăng nhập / đăng ký | |
| N.3 | Layout dashboard theo role (SV / GV / Admin) | |
| N.4 | Practice: list problem, detail, editor, submit, lịch sử | |
| N.5 | Lớp + contest UI (gắn API phần G, K) | |
| N.6 | Socket client: subscribe kết quả chấm | |
...
| # | Việc | Ghi chú |
|---|------|---------|
| O.1 | Logging có cấu trúc (request id, submission id) | |
| O.2 | Metrics queue (độ dài, latency) | |
| O.3 | Test tích hợp API chính (auth, submit happy path) | |
| O.4 | E2E một luồng: đăng nhập → nộp → worker → kết quả | |
...
### Trạng thái codebase (để không lặp task)

Các phần **đã có trong repo** (chỉ bổ sung/gắn nghiệp vụ thật, không kế hoạch lại từ đầu):

| Lớp | Đã có | Ghi chú / còn thiếu |
|-----|--------|----------------------|
| **Core API — HTTP** | CORS, `ValidationPipe` (whitelist), `AllExceptionsFilter`, `TransformResponseInterceptor`, Swagger `/api-docs` | — |
| **Core API — Auth** | `JwtAuthGuard` + `RolesGuard` global, `@Public()`, JWT ký/verify, `POST /auth/login`, `GET /auth/me` | Login **chưa** kiểm tra mật khẩu; **chưa** `register`; **chưa** check `isActive` |
| **Core API — Queue** | `BullMqModule`: `Queue` + `QueueEvents`, Redis `maxRetriesPerRequest: null`, tên queue `judge-submissions`, `BullMqEventsService` → Socket | — |
| **Core API — Submissions** | `POST /submissions` tạo bản ghi + `add` job | Đang `@Public()`, `userId` trong body, **upsert** User/Problem kiểu demo |
| **Core API — Realtime** | `SubmissionGateway` (Socket.io), room `user:<id>`, event `submission:created/progress/finished/failed` | Client join bằng **`?userId=`** — chưa JWT |
| **Worker** | Consumer BullMQ, cập nhật `Running` → kết quả, `job.updateProgress` | **Stub** luôn `Accepted`; **chưa** đọc `TestCase`; **chưa** ghi `caseResults` |
| **Prisma** | `schema.prisma` đầy đủ + migration `20260505120000_rfp_full_schema` | **Chưa** `prisma/seed.ts` |
| **Web** | `/` demo nộp bài + Socket; `/test` showcase UI; `getPublicCoreUrl()` | **Chưa** trang login/register; submit **chưa** gửi Bearer token |

**Chưa có (nên nằm trong task sprint)**: `GET /health`, seed, đăng ký + bcrypt, khóa submission bằng JWT, judge/`caseResults` thật, CI workflow, toàn bộ module Problem / Class / Contest trên API.
...
- **Đã có**: JWT, `POST /auth/login` chỉ theo email; field `User.passwordHash` trên schema.
...
- **Đã có**: field `isActive` trên model; chưa dùng trong login.
...
- **Đã có**: `getPublicCoreUrl()`, chưa auth UI.
...
- **Đã có**: script `lint`/`build` ở root workspaces; **chưa** workflow CI trong repo.
...
- **Đã có**: model Prisma; **chưa** HTTP API.
